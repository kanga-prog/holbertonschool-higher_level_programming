...
def shape_info(shape):
    ...
    This function demonstrates the use of duck typing. It does not explicitly check
    the type of the object passed but assumes the object has `area` and `perimeter`
    methods, which it calls to display the area and perimeter.

    Args:
        shape (Shape): An object of a class that inherits from the Shape abstract class.

    Raises:
        AttributeError: If the object does not have the `area` or `perimeter` methods.
    """
