...
MySQL database hbtn_0e_0_usa, safely preventing SQL injection.
...
def fetch_states_by_name(cursor, state_name):
    ...
    argument, using a parameterized query to prevent SQL injection.
...
def main():
    ...
    matching the user input for state name, securely preventing SQL injection.
    """
    if len(sys.argv) != 5:
        print("Usage: ./3-my_safe_filter_states.py <mysql username> <mysql password>"
